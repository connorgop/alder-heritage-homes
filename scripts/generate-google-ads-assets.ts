...
const PROBLEM_PROPERTY_KEYWORDS = [
  ...
  {
    adGroup: "Medical Debt & Judgment Liens",
    finalUrl: `${BASE_URL}/sell-house-to-pay-medical-debt-fresno`,
    keywords: [
      "sell house to pay medical debt fresno",
      "sell house medical bills fresno",
      "sell home to pay hospital bills fresno",
      "sell house to avoid bankruptcy fresno",
      "sell house with judgment lien fresno",
      "sell house to pay credit card debt fresno",
      "sell house with collections lien fresno",
    ],
  },
];

const SMALL_CITY_TAKEOVER_CITIES = [
...
const SMALL_CITY_TAKEOVER_KEYWORDS = [
  "sell my house fast",
  "we buy houses",
  "cash home buyer",
  "sell house as is",
  ...
  "sell house in probate",
  "sell house with tenants",
  "sell tired landlord house",
  "sell house title issues",
  "sell house deed problems",
  "sell house with liens",
  ...
  "sell house unpaid property taxes",
  "sell house medical bills",
  "direct cash buyer not wholesaler",
];
...
function main() {
  ...
  const smallCityTakeoverRows = SMALL_CITY_TAKEOVER_CITIES.flatMap(city => {
    const finalUrl = landingPage(city.slug);
    return SMALL_CITY_TAKEOVER_KEYWORDS.flatMap(base => {
      const keyword = `${base} ${city.name}`;
      return [
        ["Small City Cash Sellers", `${city.name} | High Intent`, "Exact", `[${keyword}]`, finalUrl],
        ["Small City Cash Sellers", `${city.name} | High Intent`, "Phrase", `"${keyword}"`, finalUrl],
      ];
    });
  });
  writeCsv("small-city-takeover-keywords.csv", ["Campaign", "Ad group", "Match type", "Keyword", "Final URL"], smallCityTakeoverRows);
...
`;
  ...
  console.log(`[ads-assets] ${smallCityTakeoverRows.length} small city takeover keyword rows`);
